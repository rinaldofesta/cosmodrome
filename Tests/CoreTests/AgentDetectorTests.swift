...
final class AgentDetectorTests: XCTestCase {
    ...
    func testDetectsNeedsInput() {
        let detector = AgentDetector(agentType: "claude", sessionId: UUID(), sessionName: "test", debounce: 0)
    ...
    func testGenericNeedsInput() {
        let detector = AgentDetector(agentType: "unknown", sessionId: UUID(), sessionName: "test", debounce: 0)
    ...
    func testDebounce() {
        let detector = AgentDetector(agentType: "claude", sessionId: UUID(), sessionName: "test", debounce: 10.0)
        detector.analyzeText("Error occurred")
        XCTAssertEqual(detector.state, .error)
